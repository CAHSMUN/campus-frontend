...
const HeadDelegates = () => {
    ...
    async function loadDelegates(schoolsMap) {
        try {
            let res = await axios.get(`${API_URL}/sponsors/delegates/${schoolID}`, {
                headers: {
            ...
            let functional_list = res.data.map((item) => {return {...item, school: schoolsMap[item.school],
                //  action: {school_id: item._id, detail_fn: handleDetail}
                }});
